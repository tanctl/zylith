...
pub use crate::witness::{
    ...
    generate_membership_witness_inputs, generate_swap_witness_inputs, generate_withdraw_witness_inputs,
    DepositWitnessInputs, LpWitnessInputs, MembershipWitnessInputs, SwapWitnessInputs,
...
pub async fn prove_membership(
    witness_inputs: MembershipWitnessInputs,
    circuit_dir: &Path,
) -> Result<ProofCalldata, ProverError> {
    let vk_path = circuit_dir.join("verification_key.json");
    prove_membership_with_vk(witness_inputs, circuit_dir, &vk_path).await
}

pub async fn prove_membership_with_vk(
    witness_inputs: MembershipWitnessInputs,
    circuit_dir: &Path,
    vk_path: &Path,
) -> Result<ProofCalldata, ProverError> {
    let witness_json = generate_membership_witness_inputs(witness_inputs)?;

    let wasm = circuit_dir.join("membership.wasm");
    let zkey = circuit_dir.join("membership_final.zkey");
    let snarkjs_output = generate_proof_snarkjs("membership", &witness_json, &wasm, &zkey).await?;
    let calldata = generate_garaga_calldata(
        vk_path,
        &snarkjs_output.proof_path,
        &snarkjs_output.public_inputs_path,
