...
// private swap circuit - roves ekubo swap math for bounded multi-step swaps, ashes and tick data are enforced on-chain.
// (only act-input swaps are supported in this version)
...
template PrivateSwap() {
    ...
    var VK_SWAP = 0x53574150; // "SWAP"
    var DOMAIN_TAG = 0x5a594c495448; // "ZYLITH"
    var NOTE_TYPE_TOKEN = 1;

    // public inputs: aggregate pool state only,individual note amounts stay private
    signal input tag;
    ...
    signal input step_next0_div_floor_q[MAX_STEPS][4];
    ...
    amount_remaining[0] <== note_amount_total;
    sum_in_chain[0] <== 0;
    sum_out_chain[0] <== 0;
    ...
    // if and only if there is leftover input, we must create a nonzero change note
