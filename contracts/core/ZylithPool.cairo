...
pub mod ZylithPool {
    ...
    use crate::clmm::interfaces::core::{ICoreDispatcher, ICoreDispatcherTrait, SwapParameters};
    ...
    use crate::clmm::math::ticks::{
        max_sqrt_ratio, min_sqrt_ratio, sqrt_ratio_to_tick, tick_to_sqrt_ratio,
    };
    use crate::clmm::types::bounds::Bounds;
    use crate::clmm::types::delta::Delta;
    use crate::clmm::types::i129::{AddDeltaTrait, i129};
    use crate::clmm::types::keys::PoolKey;
    use crate::constants::generated as generated_constants;
    use crate::core::PoolAdapter::{IPoolAdapterDispatcher, IPoolAdapterDispatcherTrait};
    ...
    use crate::privacy::ShieldedNotes::{
        MerkleProof, ShieldedNotesExternalDispatcher, ShieldedNotesExternalDispatcherTrait,
    };
    ...
    impl External of super::ZylithPoolExternal<ContractState> {
        fn initialize(
            ...
            assert(
                core.get_authorized_adapter() == self.pool_adapter.read(), 'CORE_ADAPTER_MISMATCH',
            );
            ...
            let notes = ShieldedNotesExternalDispatcher {
                contract_address: self.shielded_notes.read(),
            };
        ...
        fn transfer_ownership(ref self: ContractState, new_owner: ContractAddress) {
            ...
            self
                .emit(
                    OwnerTransferred {
                        old_owner, new_owner, timestamp: starknet::get_block_timestamp(),
                    },
                );
        }
        ...
        fn quote_swap_steps(self: @ContractState, params: SwapParameters) -> SwapStepsQuote {
            ...
            let (fee_growth_global_0_before, fee_growth_global_1_before) = adapter
            ...
            while step_idx < generated_constants::MAX_SWAP_STEPS {
                ...
                let active = amount_remaining.is_non_zero()
                    & (sqrt_ratio != params.sqrt_ratio_limit);
                let swap = if (active) {
                    swap_result(
                        sqrt_ratio,
                        liquidity,
        ...
        fn get_fee_growth_inside(
            self: @ContractState, tick_lower: i32, tick_upper: i32,
        ) -> (u256, u256) {
            let config = self.pool_config.read();
            let core = ICoreDispatcher { contract_address: self.core_address.read() };
            let bounds = bounds_from_ticks(tick_lower, tick_upper);
            let fees = core
                .get_pool_fees_per_liquidity_inside(pool_key_from_config(config), bounds);
    ...
    fn bounds_from_ticks(tick_lower: i32, tick_upper: i32) -> Bounds {
        ...
        Bounds { lower: lower_i128.into(), upper: upper_i128.into() }
    }
