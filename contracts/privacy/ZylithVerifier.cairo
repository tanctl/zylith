...
const VK_MEMBERSHIP: felt252 = 'MEMBERSHIP';
...
const MEMBERSHIP_PUBLIC_INPUTS_LEN: usize = 3;
...
pub trait IZylithVerifier<TContractState> {
    ...
    fn verify_membership_proof(self: @TContractState, calldata: Span<felt252>) -> Option<MembershipPublicOutputs>;
    ...
    fn update_membership_verifier(ref self: TContractState, new_address: ContractAddress);
...
pub mod ZylithVerifier {
    use super::{
        ...
        DepositPublicOutputs, MembershipPublicOutputs, VK_DEPOSIT, VK_LIQ_ADD,
        VK_LIQ_CLAIM, VK_LIQ_REMOVE, VK_MEMBERSHIP, VK_SWAP, VK_SWAP_EXACT_OUT, VK_WITHDRAW,
        WithdrawPublicOutputs, SWAP_PUBLIC_INPUTS_LEN, LIQUIDITY_BINDING_INPUTS_LEN,
        MEMBERSHIP_PUBLIC_INPUTS_LEN, DEPOSIT_PUBLIC_INPUTS_LEN, WITHDRAW_PUBLIC_INPUTS_LEN,
    ...
    fn constructor(
        ...
        assert(membership_verifier.is_non_zero(), 'VERIFIER_ZERO');
    ...
    impl VerifierImpl of IZylithVerifier<ContractState> {
        ...
        fn verify_membership_proof(self: @ContractState, calldata: Span<felt252>) -> Option<MembershipPublicOutputs> {
            let verified_inputs = self.call_verifier(self.membership_verifier.read(), calldata)?;
            self.decode_membership_outputs(verified_inputs)
        }
    ...
    fn set_verifier(ref self: ContractState, kind: felt252, address: ContractAddress) {
        ...
        } else if kind == VK_MEMBERSHIP {
            self.membership_verifier.write(address);
    ...
    impl VerifierHelpersImpl of VerifierHelpers {
        ...
        fn decode_membership_outputs(
            self: @ContractState, public_inputs: Span<u256>
        ) -> Option<MembershipPublicOutputs> {
            // public input order (including domain tag):
            // [0] proof type tag (VK_MEMBERSHIP)
            // [1] merkle_root
            // [2] commitment
            if public_inputs.len() != MEMBERSHIP_PUBLIC_INPUTS_LEN {
                return Option::None;
            }
            let tag: felt252 = assert_high_zero(*public_inputs.at(0)).try_into().expect('TAG_RANGE');
            if tag != VK_MEMBERSHIP {
                return Option::None;
            }
            Option::Some(
                MembershipPublicOutputs {
                    merkle_root: (*public_inputs.at(1)).try_into().expect('ROOT_RANGE'),
                    commitment: (*public_inputs.at(2)).try_into().expect('COMMITMENT_RANGE'),
                },
            )
        }
