...
// this avoids leaking tree structure and access patterns (no sparse node storage). delegates
// nullifier tracking and root history to dedicated components, commitments are fully
// caller-supplied.
...
pub trait ShieldedNotesExternal<TContractState> {
    ...
    fn append_commitment(
        ref self: TContractState, commitment: felt252, token: ContractAddress, proof: MerkleProof,
    ) -> u64;
    ...
    fn verify_membership(self: @TContractState, token: ContractAddress, proof: MerkleProof) -> bool;
    fn verify_position_membership(self: @TContractState, proof: MerkleProof) -> bool;
    fn withdraw_token0(ref self: TContractState, calldata: Span<felt252>, proof: MerkleProof);
    fn withdraw_token1(ref self: TContractState, calldata: Span<felt252>, proof: MerkleProof);
...
pub mod ShieldedNotes {
    ...
    use super::{
        ContractAddress, IERC20Dispatcher, IERC20DispatcherTrait, MERKLE_HEIGHT, MerkleProof,
        ROOT_BATCH_DELAY_SECS, ROOT_BATCH_SIZE, ShieldedNotesAdmin, ShieldedNotesExternal,
        assert_zero_leaf_hash, get_block_timestamp, get_contract_address, is_authorized, max_leaves,
        poseidon_hash_pair, zero_leaf_hash,
    };

    component!(
        path: NullifierRegistryComponent, storage: nullifier_registry, event: NullifierEvent,
    );
    impl Nullifiers = NullifierRegistryComponent::NullifierRegistryImpl<ContractState>;

    component!(
        path: HistoricalRootsComponent0, storage: historical_roots0, event: HistoricalRootsEvent0,
    );
    impl Roots0 = HistoricalRootsComponent0::HistoricalRootsImpl<ContractState>;

    component!(
        path: HistoricalRootsComponent1, storage: historical_roots1, event: HistoricalRootsEvent1,
    );
    impl Roots1 = HistoricalRootsComponent1::HistoricalRootsImpl<ContractState>;

    component!(
        path: HistoricalRootsComponentPosition,
        storage: historical_roots_position,
        event: HistoricalRootsEventPosition,
    );
    ...
    // Inserts commitment into the token-specific tree, returns leaf index, batches root history
    // updates.
    // The caller supplies a Merkle proof for the empty leaf at next_index so we can update roots
    // without storing internal nodes on-chain.
    fn insert_token_commitment(
        ref self: ContractState, commitment: felt252, token: ContractAddress, proof: MerkleProof,
    ) -> u64 {
        ...
        let old_root = compute_root_from_path(
            zero_leaf_hash(), proof.leaf_index, proof.path, proof.indices,
        );
        assert(old_root == current_root, 'PROOF_INVALID');
        let new_root = compute_root_from_path(
            commitment, proof.leaf_index, proof.path, proof.indices,
        );
        if token == token0 {
        ...
        self.emit(Deposit { commitment, leaf_index, token, timestamp: get_block_timestamp() });
        ...
        self
            .emit(
                PositionCommitmentInserted {
                    commitment, leaf_index, timestamp: get_block_timestamp(),
                },
            );
    ...
    fn record_root_batch_token(ref self: ContractState, token: ContractAddress, new_root: felt252) {
    ...
    fn is_known_root_internal(self: @ContractState, token: ContractAddress, root: felt252) -> bool {
        let token0 = self.token0.read();
        let token1 = self.token1.read();
        assert((token == token0) | (token == token1), 'TOKEN_NOT_ALLOWED');
    ...
    fn verify_membership_proof(
        ...
        let current =
            match try_compute_root_from_path(
    ...
    fn verify_position_membership_proof(self: @ContractState, proof: MerkleProof) -> bool {
        if (proof.commitment == 0) | (proof.commitment == zero_leaf_hash()) {
            return false;
        }
    ...
    impl ExternalImpl of ShieldedNotesExternal<ContractState> {
        ...
        fn withdraw_token0(ref self: ContractState, calldata: Span<felt252>, proof: MerkleProof) {
            self.flush_pending_roots();
            let token = self.token0.read();
            self.handle_withdraw(calldata, proof, token, 0);
        }

        fn withdraw_token1(ref self: ContractState, calldata: Span<felt252>, proof: MerkleProof) {
            self.flush_pending_roots();
            let token = self.token1.read();
            ...
            self
                .emit(
                    AuthorizedPoolUpdated {
                        old_pool, new_pool: authorized_pool, timestamp: get_block_timestamp(),
                    },
                );
        }
    ...
    impl ShieldedNotesHelpersImpl of ShieldedNotesHelpers {
        ...
        fn handle_withdraw(
            ...
            assert(
                IERC20Dispatcher { contract_address: token }
                    .transfer(outputs.recipient, outputs.amount),
                'TRANSFER_FAILED',
            );
